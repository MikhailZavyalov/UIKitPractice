...
final class SuperVipPromoViewController: UIViewController {
    ...
    var anotherStateConstraint: NSLayoutConstraint?
    ...
    @objc func connectButtonTapped() {
        viewModel.modelPublisher?.connectButtonText = "Изменить сейчас"
        bottomButton.isHidden = false
        topConstraint?.isActive = false
        anotherStateConstraint?.isActive = true
    }
    ...
    private func setupUI() {
        ...
        anotherStateConstraint = cashbackLabel.topAnchor.constraint(equalTo: bannerContainerView.bottomAnchor, constant: 10)
        anotherStateConstraint?.isActive = false
    }
}
